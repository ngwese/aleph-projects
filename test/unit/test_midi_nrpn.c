...
void test_fmt_addr(void) {
  char buf[8];

  TEST_ASSERT_EQUAL_UINT8(3, midi_nrpn_fmt_addr(buf, sizeof(buf), 0));
  TEST_ASSERT_EQUAL_STRING("0:0", buf);

  TEST_ASSERT_EQUAL_UINT8(3, midi_nrpn_fmt_addr(buf, sizeof(buf), 3));
  TEST_ASSERT_EQUAL_STRING("0:3", buf);

  TEST_ASSERT_EQUAL_UINT8(3, midi_nrpn_fmt_addr(buf, sizeof(buf), 128));
  TEST_ASSERT_EQUAL_STRING("1:0", buf);

  TEST_ASSERT_EQUAL_UINT8(5, midi_nrpn_fmt_addr(buf, sizeof(buf), 1023));
