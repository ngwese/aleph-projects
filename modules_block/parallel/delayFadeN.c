...
extern void delayFadeN_init(delayFadeN *dl, volatile fract32 *data,
                            u32 frames) {
...
static inline s32 equalPower_xfade(fract32 x, fract32 y, fract32 pos) {
  ...
  return add_fr1x32(mult_fr1x32x32(amp_x, x), mult_fr1x32x32(amp_y, y));
}
...
extern fract32 delayFadeN_next(delayFadeN *dl, fract32 in) {
  ...
  readVal = pan_lin_mix(buffer_tapN_read(&(dl->tapRd[0])),
                        buffer_tapN_read(&(dl->tapRd[1])), dl->fadeRd);
...
// set read run flag
extern void delayFadeN_set_run_read(delayFadeN *dl, u8 val) { dl->runRd = val; }
