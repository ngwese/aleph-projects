...
s32 scaler_labels_val(void *scaler, io_t in) { return (io_t)in; }

char scaler_labels[MAX_LABELS][LABEL_LENGTH] = {
    "EFFECT", "ADC0",   "ADC1", "ADC2", "ADC3", "ADC4", "GRAIN1", "GRAIN2",
    "PTOSC1", "PTOSC2", "ENV1", "ENV2", "LFO",  "PING", "CV1",    "CV2"};
...
void scaler_parse_labels_char(char c) {
  ...
  if (labelsWriteHead - &scaler_labels[scaler_n_labels][0] >=
      LABEL_LENGTH - 1) {
    *labelsWriteHead = 0;
