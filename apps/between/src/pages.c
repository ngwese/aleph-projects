...
u8 app_mode_is_inspect(void) { return (u8)(g_app_mode == eAppModeInspect); }
...
void pages_init(void) {
  ...
  g_pages[ePageSetups] = (Page){
      .name = "setups", .select_fn = select_setups, .redraw_fn = redraw_setups};
  g_pages[ePageModules] = (Page){.name = "modules",
                                 .select_fn = select_modules,
                                 .redraw_fn = redraw_modules};
  g_pages[ePageSlots] = (Page){
      .name = "slots", .select_fn = select_slots, .redraw_fn = redraw_slots};
  g_pages[ePageSlotA] = (Page){
      .name = "slot a", .select_fn = select_slot_a, .redraw_fn = redraw_slot_a};
  g_pages[ePageSlotB] = (Page){
      .name = "slot b", .select_fn = select_slot_b, .redraw_fn = redraw_slot_b};
  g_pages[ePageSlotC] = (Page){
      .name = "slot c", .select_fn = select_slot_c, .redraw_fn = redraw_slot_c};
  g_pages[ePageSlotD] = (Page){
      .name = "slot d", .select_fn = select_slot_d, .redraw_fn = redraw_slot_d};
  g_pages[ePagePlayMaps] = (Page){.name = "play",
                                  .select_fn = select_play_maps,
                                  .redraw_fn = redraw_play_maps};
  g_pages[ePageInfo] = (Page){
      .name = "info", .select_fn = select_info, .redraw_fn = redraw_info};
  g_pages[ePagePlay] = (Page){
      .name = "play", .select_fn = select_play, .redraw_fn = redraw_play};
  g_pages[ePageInspect] = (Page){.name = "inspect",
                                 .select_fn = select_inspect,
