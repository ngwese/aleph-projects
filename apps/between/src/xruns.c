...
const bfin_xrun_t *xruns_get(void) { return &g_xruns; }
...
u8 xruns_poll(void) {
  ...
  bfin_get_xruns(&cur);
  changed =
      (cur.windowRx != g_xruns.windowRx || cur.windowTx != g_xruns.windowTx ||
       cur.clashRx != g_xruns.clashRx || cur.clashTx != g_xruns.clashTx);
