...
SetupIoStatus setup_io_write(LineIO *io, const SetupData *data) {
  char ver[32];
  char num[24];
  static const char *slot_keys[MORPH2D_SLOTS] = {"slot.a", "slot.b", "slot.c",
                                                 "slot.d"};
  u32 i;
  ...
  for (i = 0; i < MORPH2D_SLOTS; ++i) {
    const char *stem = data->slot_occupied[i] ? data->slot_stem[i] : "-";
  ...
  }
  {
    ...
    static const char *sw_keys[PLAY_MAPS_SW_COUNT] = {"play.sw0", "play.sw1",
                                                      "play.sw2", "play.sw3"};
    static const char *fs_keys[PLAY_MAPS_FS_COUNT] = {"play.fs0", "play.fs1"};
    static const char *cv_keys[PLAY_MAPS_CV_COUNT] = {"play.cv0", "play.cv1",
                                                      "play.cv2", "play.cv3"};
    static const char *cc_keys[PLAY_MAPS_CC_COUNT] = {
        "play.cc1", "play.cc2",  "play.cc3",  "play.cc4",
        "play.cc5", "play.cc6",  "play.cc7",  "play.cc8",
        "play.cc9", "play.cc10", "play.cc11", "play.cc12"};
